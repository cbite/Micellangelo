...
NucleiLocationOneFeature=TimprovedByRank-100;
...
meshInterest = [11:10:91];
averageDapiImage = uint16(zeros(101,101, num_channels));
for i = 1:numel(meshInterest)
    ...
    stackDapiImage = uint16(zeros(101,101, num_channels));
    ...
    for j = 1:numel(nucleiInsideMesh)
        ...
        middle=median(1:length(dapiImage));
        dapiImage = dapiImage(middle-sizeFeatureInPixels:middle+sizeFeatureInPixels, middle-sizeFeatureInPixels:middle+sizeFeatureInPixels);
...
for i = 1:size(averageDapiImage,3)
    ...
    middle = round(length(averageDapiImage)/2);
    cropxy = 25; % 25 pixels
...
filename = "Meshed_meanNucleiSlow.gif"; % Specify the output file name
...
filename = "MeshedmeanNucleiFast.gif"; % Specify the output file name
for idx = 1:numel(meshInterest)
    [A,map] = rgb2ind(im{idx},256);
    if idx == 1
